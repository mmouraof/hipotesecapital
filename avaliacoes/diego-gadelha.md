...
1. **Import order bug**: `load_dotenv()` chamado apos importar LLM → chave Groq nunca carregada → LLM sempre falha
2. **`salvar_snapshot_no_db()` com corpo `pass`** → nenhum dado temporal persiste → LLM nao consegue construir prompts
